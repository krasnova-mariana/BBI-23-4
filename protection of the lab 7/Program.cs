...
namespace _7_lab_2
{
    class Program
    {
        static void Main()
        {
            ...
            for (int i = 0; i < a.Length; i++)
                a[i].calcSum();
            a = a[0].QuickSort(a, 0, a.Length - 1);
    ...
    abstract class Wintersport
    {
        ...
        public int FindPivot(Wintersport[] a, int minIndex, int maxIndex)
        {
            int pivot = minIndex - 1;
            Wintersport tmp;
            for (int i = minIndex; i < maxIndex; i++)
            {
                if (a[i].sum < a[maxIndex].sum)
                {
                    pivot++;
                    tmp = a[pivot];
                    a[pivot] = a[i];
        ...
        public Wintersport[] QuickSort(Wintersport[] a, int minIndex, int maxIndex)
        {
